...
def my_scrape():
    for entry in html.find_all(['span', 'div', 'i'], {'class': ['sr-hotel__name', 'bui-review-score__badge', 'bk-icon-wrapper bk-icon-stars star_track', 'strike-it-red_anim change-text-color-gray']}):
        ...
        if match:
            ...
            hotel_dict[temp_key][0] = temp_value
        ...
        if match:
            ...
            hotel_dict[temp_key][1] = temp_value
        ...
        if match:
            ...
            if str(entry):
                ...
                temp_value = re.search(r'\d+', temp_value)[0]   #some text-processing to extract a number
                hotel_dict[temp_key][2] = temp_value
...
pdtable = pd.DataFrame.from_dict(hotel_dict, orient='index', columns=['Rating', 'Star', 'Price'])
                       
print(pdtable)

#save the data in csv format
pdtable.to_csv('hotel_prices.csv')
